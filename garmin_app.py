...
async def obtener_carga_trabajo_con_grafico():
    ...
    texto = (
        ...
        f"📅 Fecha: `{today_str}`\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🛡️ *Fitness (CTL):* `{ctl_actual}` _(Base 42 días - Buscar subir)_\n"
        ...
        f"• `-30 a -10`: 🏋️ *Carga Óptima* (Estimulo para mejorar)\n"
        f"• `< -30`: ⚠️ *Riesgo Alto* (Sobreentrenamiento)\n\n"
        f"📌 *¿CÓMO LEER EL GRÁFICO?*\n"
        f"• 🔷 *Línea Azul (CTL):* Acumulación de carga a largo plazo.\n"
        f"• 🟧 *Línea Naranja (ATL):* Carga a corto plazo (últimos 7 días).\n"
        f"• 🟢 *Línea Verde (TSB):* Tu frescura actual (CTL - ATL)."
    )
